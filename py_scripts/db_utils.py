...
def get_unique_dates(data_folder:str):
    ...
    а написано, что "файлы за три дня", не знаю, как будет проверяться, поэтому 
    пришлось поупражняться, чтобы корректно обрабатывалось в любом случае. Если файлы даются как по 
    условию -  по 1 "комплекту" за каждый день каждый день, то такая сложная обработка не нужна
...
def process_by_day(lst: list, 
    ...
    :param archive_folder: каталог для обработанных файлов (создастся если еще нет)
    ...
    """
    #запускаем поиск фалов с датами в именах
    ...
    for dt in date_obj:
        ...
        cnt=0 #для корректной загрузки нужны 3 файла, начинаем считать
        
        for name in lst:
            # Ищем файл конкретного типа транзакции/терминалы/блек_лист за конкретную дату
            ...
            try:
                ...
                #сохраняем лог в базе на память
        ...
        if cnt != 3:
            ...
            #в случае, если файлов достаточно, запускаем скрипт обработки и построения витрины
